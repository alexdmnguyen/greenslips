...
export async function generateMetadata({ params }: SportBettingProps): Promise<Metadata> {
  const sport = sports.find((s) => s.slug === params.slug && s.category === 'sports');
  if (!sport) {
    return { title: 'Sport Not Found - SAMBO SENTRAL' };
  }
  return {
    title: `${sport.name} Betting - SAMBO SENTRAL`,
    description: `Betting hub for ${sport.name}.`,
  };
}

export default function SportBettingPage({ params }: SportBettingProps) {
  const sport = sports.find((s) => s.slug === params.slug && s.category === 'sports');
  if (!sport) {
    notFound();
  }
  ...
  const sportArticles = articles.filter(
    (a) => a.categories?.includes(`${sport.slug}-betting`)
  );
    ...
    <div className="min-h-screen py-16 px-4">
      <div className="max-w-6xl mx-auto space-y-12">
        <div className="text-center">
          <Wallet className="h-14 w-14 text-blue-500 mx-auto mb-4" />
          <h1 className="text-5xl font-bold text-white mb-3">{sport.name} Betting</h1>
          <p className="text-lg text-gray-400">
            Odds movement, value spots, and bankroll-friendly angles for {sport.name}.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {sportArticles.length === 0 && (
            <div className="col-span-full text-center text-gray-400">
              No betting notes yet. Check back soon.
            </div>
          )}
