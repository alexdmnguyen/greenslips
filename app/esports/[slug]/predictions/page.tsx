...
interface EsportPredictionsProps {
  params: { slug: string };
}
...
export async function generateMetadata({ params }: EsportPredictionsProps): Promise<Metadata> {
  const sport = sports.find((s) => s.slug === params.slug && s.category === 'esports');
  if (!sport) {
    return { title: 'Esport Not Found - SAMBO SENTRAL' };
  }
  return {
    title: `${sport.name} Predictions - SAMBO SENTRAL`,
    description: `Predictions hub for ${sport.name}.`,
  };
}

export default function EsportPredictionsPage({ params }: EsportPredictionsProps) {
  const sport = sports.find((s) => s.slug === params.slug && s.category === 'esports');
  if (!sport) {
    notFound();
  }
  ...
  const sportArticles = articles.filter(
    (a) => a.categories?.includes(`${sport.slug}-predictions`)
  );
    ...
    <div className="min-h-screen py-16 px-4">
      <div className="max-w-6xl mx-auto space-y-12">
        <div className="text-center">
          <Lightbulb className="h-14 w-14 text-blue-500 mx-auto mb-4" />
          <h1 className="text-5xl font-bold text-white mb-3">{sport.name} Predictions</h1>
          <p className="text-lg text-gray-400">
            Series outlooks, map reads, and meta calls for {sport.name}.
          </p>
        </div>
