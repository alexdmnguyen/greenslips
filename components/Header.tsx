  ...
  let navigation: NavItem[] = [
    {
      name: 'Sports Hub',
      href: '/sports',
      children: sportsNav.map((sport) => ({ name: sport.name, href: `/sports/${sport.slug}` })),
    },
    {
      name: 'Esports Hub',
      href: '/esports',
      children: esportsNav.map((sport) => ({ name: sport.name, href: `/esports/${sport.slug}` })),
    },
    {
      name: 'Daily Picks',
      href: '/picks',
...
<<<<<<< ours
  const activeSection =
    activeCategory === 'esports' ? 'esports' : activeCategory === 'sports' ? 'sports' : null;
        ...
        <div className="relative flex h-16 items-center justify-between">
          <div className="flex items-center">
            <Link href="/" className="flex items-center space-x-2">
              ...
              <span className="text-xl font-bold text-white">SAMBO SENTRAL</span>
            </Link>
          </div>

          <div className="hidden md:flex flex-1 items-center justify-between ml-3">
            <div className="flex rounded-full border border-slate-700 bg-slate-800/70 p-0.5">
                <Link
                  href="/sports"
...
>>>>>>> theirs
                    ...
                    {item.children && (
                      <div className="invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-all duration-150 absolute left-0 top-full mt-1 min-w-[180px] rounded-b-lg rounded-t-none border border-t-0 border-slate-800 bg-slate-900/95 shadow-xl backdrop-blur-sm p-2 z-40">
